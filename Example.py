...
if __name__ == '__main__':
    ...
    ax.plot(lim, linear.intercept_ + linear.coef_ * lim, 'r--', linewidth=2, label='Linear fit')
    ax.plot(x, quad.model.intercept_ + quad.model.coef_ * np.square(x), 'g-.', linewidth=2, label='Quadratic fit')
    ax.set_xlabel('x')
    ax.set_ylabel('Y')
    ax.legend(loc=2)
    f.savefig('img/Singles.pdf', bbox_inches='tight')

    # Multiple models
    stack = LinearStack(estimators=[LinearRegression(), QuadReg()], verbose=2).fit(x.reshape(-1, 1), y)
    ...
    ax.set_ylabel('Y')
    ax.legend(loc=2)
    f.savefig('img/Stackings.pdf', bbox_inches='tight')
